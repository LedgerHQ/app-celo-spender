 ...
 *
 * @param a The array to compute the size of.
 ...
 * @return offset if successful, -1 if failed
 */
 ...
 *
 * This function examines a buffer of specified length and determines whether
 * all bytes in the buffer are zero. It performs a byte-by-byte comparison
 * and returns early if any non-zero byte is found.
 *
 * @param buf A pointer to the buffer to be checked.
 * @param n The length of the buffer in bytes.
 * @return 1 if all bytes are zero, 0 if any non-zero byte is found.
 */
 ...
 *
 * This function reads a big-endian byte array and converts it to a 64-bit
 * unsigned integer. The function processes bytes from left to right (most
 * significant byte first) and builds the result by shifting and combining
 * the bytes. If the input size is larger than 8 bytes, only the first 8
 * bytes are processed.
 *
 * @param in A pointer to the big-endian byte array to convert.
 * @param size The number of bytes in the input array.
