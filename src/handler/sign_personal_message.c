...
int handleSignPersonalMessage(uint8_t p1,
    ...
    if (p1 == P1_FIRST) {
        ...
        cx_err_t err = cx_keccak_init_no_throw(&sha3, 256);
        if (err != CX_OK) {
            return io_send_sw(err);
        }
        err = cx_hash_no_throw((cx_hash_t *) &sha3,
                               0,
                               (uint8_t *) SIGN_MAGIC,
                               sizeof(SIGN_MAGIC) - 1,
                               NULL,
        ...
        err = cx_hash_no_throw((cx_hash_t *) &sha3, 0, (uint8_t *) tmp, pos, NULL, 0);
        if (err != CX_OK) {
            return io_send_sw(err);
        }
    ...
    cx_err_t err = cx_hash_no_throw((cx_hash_t *) &sha3, 0, workBuffer, dataLength, NULL, 0);
    if (err != CX_OK) {
        return io_send_sw(err);
    }
    err = cx_hash_no_throw((cx_hash_t *) &tmpContent.sha2, 0, workBuffer, dataLength, NULL, 0);
    if (err != CX_OK) {
        return io_send_sw(err);
    }
    ...
    if (tmpCtx.messageSigningContext.remainingLength == 0) {
        ...
        err = cx_hash_no_throw((cx_hash_t *) &tmpContent.sha2,
                               CX_LAST,
                               workBuffer,
                               0,
                               hashMessage,
