...
static bool prepare_infos(nbgl_contentInfoList_t *infos) {
    ...
    if (!getEthDisplayableAddress((uint8_t *) get_contract_addr(), tmp_buf, tmp_buf_size, 42220)) {
        return false;
    }
...
#ifdef SCREEN_SIZE_WALLET
    snprintf(tmp_buf, tmp_buf_size, "%s transaction to %s?", "Sign", get_operation_type());
#else
    snprintf(tmp_buf, tmp_buf_size, "%s transaction", "Sign");
#endif
