...
class TxSimu(TlvSerializable):
    ...
    def serialize(self) -> bytes:
        ...
        payload += self.serialize_field(
            FieldTag.W3C_TINY_URL, self.tiny_url.encode("utf-8")
        )
        if self.chain_id:
            payload += self.serialize_field(
                FieldTag.CHAIN_ID, self.chain_id.to_bytes(8, "big")
            )
        if self.domain_hash:
            payload += self.serialize_field(FieldTag.DOMAIN_HASH, self.domain_hash)
        if self.provider_message:
            payload += self.serialize_field(
                FieldTag.W3C_PROVIDER_MSG, self.provider_message.encode("utf-8")
            )

        # Append the data Signature
        payload += self.serialize_field(
            FieldTag.DER_SIGNATURE, sign_data(Key.WEB3_CHECK, payload)
        )
        return payload
