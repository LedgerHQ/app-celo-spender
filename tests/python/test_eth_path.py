...
# Derive address & sign data tests

@pytest.mark.parametrize("show", [False, True])
@pytest.mark.parametrize("chaincode", [False, True])
def test_celo_derive_address_with_eth_path(test_name, backend, firmware, show, chaincode, navigator):
    ...
    with celo.derive_address_async(ETH_PACKED_DERIVATION_PATH, show, chaincode):
        if show:
...
def test_sign_data_with_eth_path(test_name, backend, firmware, navigator):
    celo = CeloClient(backend)
    ...
    with celo.sign_data_async(ETH_PACKED_DERIVATION_PATH, "1234567890"):
...
def sign_transaction_with_eth_path(test_name, backend, navigator, instructions, payload):
    celo = CeloClient(backend)
    with celo.sign_transaction_async(ETH_PACKED_DERIVATION_PATH,
            [
                "1234",                  #Nonce
                "1234",                  #GasPrice
                "1234",                  #StartGas
...
def sign_transaction_no_gtw_with_eth_path(test_name, backend, navigator, instructions, payload):
    celo = CeloClient(backend)
    with celo.sign_transaction_async(ETH_PACKED_DERIVATION_PATH,
            [
                "1234",                  #Nonce
...
def test_sign_transaction_empty_with_eth_path(test_name, backend, firmware, navigator):
    if firmware.device == "nanos":
    ...
    sign_transaction_with_eth_path(test_name, backend, navigator, instructions, payload)


def test_sign_transaction_token_transfer_with_eth_path(test_name, backend, firmware, navigator):
    if firmware.device == "nanos":
...
def test_sign_transaction_lock_with_eth_path(test_name, backend, firmware, navigator):
    if firmware.device == "nanos":
        instructions = get_nano_review_instructions(6)
    elif firmware.device.startswith("nano"):
        instructions = get_nano_review_instructions(4)
...
def test_sign_transaction_vote_with_eth_path(test_name, backend, firmware, navigator):
    if firmware.device == "nanos":
        instructions = get_nano_review_instructions(14)
    elif firmware.device.startswith("nano"):
        instructions = get_nano_review_instructions(6)
...
def test_sign_transaction_activate_with_eth_path(test_name, backend, firmware, navigator):
    if firmware.device == "nanos":
...
def test_sign_transaction_revoke_with_eth_path(test_name, backend, firmware, navigator):
    if firmware.device == "nanos":
        instructions = get_nano_review_instructions(14)
    elif firmware.device.startswith("nano"):
        instructions = get_nano_review_instructions(6)
...
def test_sign_transaction_unlock_with_eth_path(test_name, backend, firmware, navigator):
    if firmware.device == "nanos":
...
def test_sign_transaction_withdraw_with_eth_path(test_name, backend, firmware, navigator):
    if firmware.device == "nanos":
        instructions = get_nano_review_instructions(5)
    elif firmware.device.startswith("nano"):
...
def test_sign_transaction_relock_with_eth_path(test_name, backend, firmware, navigator):
    if firmware.device == "nanos":
        instructions = get_nano_review_instructions(11)
    elif firmware.device.startswith("nano"):
        instructions = get_nano_review_instructions(5)
...
def test_sign_transaction_create_with_eth_path(test_name, backend, firmware, navigator):
    if firmware.device == "nanos":
        instructions = get_nano_review_instructions(5)
    elif firmware.device.startswith("nano"):
        instructions = get_nano_review_instructions(3)
...
def test_sign_transaction_no_gtw_with_eth_path(test_name, backend, firmware, navigator):
    if firmware.device == "nanos":
        instructions = get_nano_review_instructions(8)
    elif firmware.device.startswith("nano"):
        instructions = get_nano_review_instructions(4)
    ...
    payload = ""
    sign_transaction_no_gtw_with_eth_path(test_name, backend, navigator, instructions, payload)


def test_sign_transaction_no_gtw2_with_eth_path(test_name, backend, firmware, navigator):
    if firmware.device == "nanos":
