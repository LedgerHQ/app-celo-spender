...
static bool encode_and_hash_field(const void *const field_ptr) {
    ...
    if (!format_hash_field_type(field_ptr, (cx_hash_t *) &sha3)) {
        return false;
    }
    // space between field type name and field name
    hash_byte(' ', (cx_hash_t *) &sha3);

    // field name
    name = get_struct_field_keyname(field_ptr, &length);
    hash_nbytes((uint8_t *) name, length, (cx_hash_t *) &sha3);
    return true;
}
...
static bool encode_and_hash_type(const void *const struct_ptr) {
    ...
    hash_nbytes((uint8_t *) struct_name, struct_name_length, (cx_hash_t *) &sha3);
...
bool type_hash(const char *const struct_name, const uint8_t struct_name_length, uint8_t *hash_buf) {
    ...
    CX_CHECK(cx_keccak_init_no_throw(&sha3, 256));
    ...
    CX_CHECK(
        cx_hash_no_throw((cx_hash_t *) &sha3, CX_LAST, NULL, 0, hash_buf, KECCAK256_HASH_BYTESIZE));
    return true;
end:
    return false;
}
